...
class Fcma:
    ...
    def solve(self, solving_pars: SolvingPars = None) -> Solution:
        ...
        if FcmaStatus.is_valid(self._solving_stats.pre_allocation_status):
            ...
            # Set the pre_allocation_lower_bound_cost for speed level 1
            if speed_level == 1:
                best_bound = self._lp_problem.bestBound
                if best_bound is None:
                    # Patched CBC function does not provide a lower bound when the solution is optimal.
                    # The lower bound coincides with the pre-allocation cost.
                    best_bound = self._solving_stats.pre_allocation_cost
                else:
                    # When the patched CBC function provides a lower bound means that the solution is feasible
